...
import mapi from '../../config/api/mapi';
import Swiper from 'react-native-swiper';
import mock from '../../assets/mock/home';
...
class TabBarView extends PureComponent<Props, State> {
  constructor(props: Object) {
    ...
    this.state.article_container_top_left =
      WINDOW_WIDTH / 2 - this.state.article_container_height / 2;
    if (this.props.categoryID !== '0') {
      this.state[this.props.categoryID] = {
        isload: false,
        crtSubCategoryName:
          this.props.subCategoryList.length > 0
            ? this.props.subCategoryList[0]['categoryName']
            : '',
        crtSubCategoryId:
          this.props.subCategoryList.length > 0
            ? this.props.subCategoryList[0]['categoryID']
            : '',
  ...
  getData_ByIndex = src => {
    ...
    // Http.POST(ITENLApi.mapi.home.index, {}).done(res => {
    Http.POST(ITENLApi.apiplus.home.showV1, {
      ...
      if (res) {
        ...
        if (
          ...
          Object.keys(obj.data_first_index.article).length === 0
        ) {
          obj.data_first_index.article = {
            iconUrl:
              'http://img0.imgtn.bdimg.com/it/u=1336615893,125462541&fm=15&gp=0.jpg',
            ext: {
        ...
        if (src === 1) {
          //obj.data_first_index.apkURL = Math.random().toString();
          setTimeout(() => {
            this.setState(obj);
          }, 1000);
        } else {
  ...
  getData_ByLike = () => {
    ...
    if (this.state.data_first_moredata.isNext) {
      // Http.POST(ITENLApi.mapi.home.likeData, {
      Http.POST(ITENLApi.apiplus.home.recommend, {
        data: {
        ...
        if (res) {
          ...
          if (result.lovelyList && result.lovelyList.length > 0) {
            ...
            this.setState(preState => {
              ...
              // );
              result.lovelyList = [
                ...preState.data_first_moredata.lovelyList,
                ...result.lovelyList
        ...
        <View key={i} title={<Text numberOfLines={1}>{item.title}</Text>}>
          ...
          >
            ...
            <Image
              ...
              source={
                typeof item.uri === 'number' ? item.uri : { uri: item.uri }
              }
            />
          </TouchableOpacity>
        </View>
  ...
  renderFirstHeader = () => {
    return (
      <View>
        {this.state.data_first_index &&
        this.state.data_first_index.banners.length > 0 ? (
        ...
        <View style={styles.guarantee}>
          {this.state.data_first_index &&
          this.state.data_first_index.platformAssurance.length > 0
            ? this.state.data_first_index.platformAssurance.map((item, i) => {
                return (
                  ...
                  >
                    ...
                    />
                    <Text style={{ color: color.base, fontSize: 14 }}>
                      {item}
                    </Text>
                  </View>
                );
                  ...
                  return (
                    ...
                    >
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          alert(
                            JSON.stringify(this.state.data_first_index.article)
                          );
                        }}
                      >
        ...
        {this.state.data_first_index &&
        this.state.data_first_index.operationNavigation.length > 0 ? (
        ...
        <View style={styles.specialList}>
          ...
          {this.state.data_first_index &&
          this.state.data_first_index.specialList &&
          this.state.data_first_index.specialList.length > 0
              ...
              extView={
                ...
                >
                  <Text style={{ textAlign: 'center', color: '#808080' }}>
                    {this.state[this.props.categoryID].crtSubCategoryName}
                  </Text>
                </View>
              }
            />
